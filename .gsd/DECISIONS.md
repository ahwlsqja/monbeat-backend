...
| D033 | M006-S04 | testing | NestJS E2E mock isolation — separate TestingModule per describe block | Conflict Analysis E2E uses independent TestingModule with EngineService override + real CompileService + mocked PrismaService | Mock leak 방지. Conflict Analysis는 controlled conflict_details 필요하지만, 기존 테스트는 real EngineService 사용. describe 블록별 독립 모듈이 유일한 안전한 격리 방법. jest.fn() 타입 체크로 격리 검증 | No |
